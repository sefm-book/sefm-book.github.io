...
MSG2_CONT = {Enc.pka.(Sq.<User.b, Nonce.na, Nonce.nb>) | pka <- PKEY, b <- USER, na <- NONCE, nb <- NONCE }
...
User_A = [] b : diff(USER,{A}) @
      ...
      -> [] nb : NONCE @
         recv.A.b.(Msg2, Enc.PKA.(Sq.<User.b, Nonce.NA, Nonce.nb>))
      -> commit.A.b.NA
      -> running.A.b.nb
...
User_B = [] na : NONCE, a : diff(USER,{B}) @
      ...
      -> running.B.a.na
      -> send.B.a.(Msg2, Enc.pk(a).(Sq.<User.B, Nonce.na, Nonce.NB>))
...
Fact = Union({
             ...
             { Enc.k.(Sq.<User.b, Nonce.n, Nonce.n'>) | k<-PKEY, b <-USER, n <- NONCE, n' <- NONCE},
...
Msg2_Encryption_Deduction_Rules =
  { (Enc.k.(Sq.<User.b, Nonce.n, Nonce.n'>), {Key.k, Nonce.n, Nonce.n'}) | k<-PKEY, b <- USER, n <- NONCE, n' <- NONCE }
...
Msg2_Decryption_Deduction_Rules =
  { (Nonce.n , {Enc.k.(Sq.<User.b, Nonce.n, Nonce.n'>), Key.sk(k)}),
    (Nonce.n', {Enc.k.(Sq.<User.b, Nonce.n, Nonce.n'>), Key.sk(k)}),
	(User.b  , {Enc.k.(Sq.<User.b, Nonce.n, Nonce.n'>), Key.sk(k)}) | k<-PKEY, b <- USER, n <- NONCE, n' <- NONCE }
...
Intruder2(K) = 
  ([] F : diff(Fact, K) @ 
  (
  (member(F, MSG1_CONT) & [] x : diff(USER, {I}), y : USER @ send.x.y.(Msg1,F) -> Intruder2(union(K,{F})))
  []
  (member(F, MSG2_CONT) & [] x : diff(USER, {I}), y : USER @ send.x.y.(Msg2,F) -> Intruder2(union(K,{F})))
  []
  (member(F, MSG3_CONT) & [] x : diff(USER, {I}), y : USER @ send.x.y.(Msg3,F) -> Intruder2(union(K,{F})))
  []
  ([] r : rules_match_head(F,Deduction_Rules) @ empty(diff(rule_body(r),K)) & infer.F.r -> Intruder2(union(K,{F})))
  )
  )
  []
  (
  ...
  (
    member(F, MSG1_CONT) & (
                             ( [] x : USER, y : USER @ send.x.y.(Msg1,F) -> Intruder2(K))
                             []
                             ( [] x : USER, y : USER @ recv.x.y.(Msg1,F) -> Intruder2(K))
						   )
  )
  []
  (
    member(F, MSG2_CONT) & (
                             ( [] x : USER, y : USER @ send.x.y.(Msg2,F) -> Intruder2(K))
                             []
                             ( [] x : USER, y : USER @ recv.x.y.(Msg2,F) -> Intruder2(K))
						   )
  )
  []
  (
    member(F, MSG3_CONT) & (
                             ( [] x : USER, y : USER @ send.x.y.(Msg3,F) -> Intruder2(K))
                             []
                             ( [] x : USER, y : USER @ recv.x.y.(Msg3,F) -> Intruder2(K))
						   )
  )
  )
  )
